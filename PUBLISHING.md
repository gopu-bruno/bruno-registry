...
|---|---|---|
| **List a collection** | A PR adding `collection/<provider>/<owner>/<name>.json` with its first version | once |
...
# ns is "<provider>:<owner>", e.g. github:acme
PROVIDER=github OWNER=<owner>
mkdir -p collection/$PROVIDER/$OWNER
$EDITOR collection/$PROVIDER/$OWNER/<name>.json   # see schema/ + the example in CONTRIBUTING.md
git checkout -b add-$OWNER-<name>
git add collection/$PROVIDER/$OWNER/<name>.json
git commit -m "Add $PROVIDER:$OWNER/<name> collection"
git push -u origin add-$OWNER-<name>
gh pr create --base main --title "Add $PROVIDER:$OWNER/<name>"
```
