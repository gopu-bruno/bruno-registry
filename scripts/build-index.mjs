// Builds index.json from every collection/<provider>/<owner>/<name>.json file.
...
const REQUIRED = ['ns', 'name', 'title', 'category', 'summary', 'versions'];
const SOURCE_TYPES = ['git', 'url'];
const VERSION_TRACKING = ['git_tags', 'manual'];
// Namespace is a git identity "<provider>:<owner>"; the on-disk path expands it to
// collection/<provider>/<owner>/<name>.json.
...
function validate(entry, relPath) {
  ...
  if (!NS_RE.test(entry.ns)) {
    throw new Error(`${where}: ns "${entry.ns}" must be "<provider>:<owner>" (e.g. github:acme)`);
  }
  if (entry.version_tracking !== undefined && !VERSION_TRACKING.includes(entry.version_tracking)) {
    throw new Error(`${where}: version_tracking must be one of ${VERSION_TRACKING.join(', ')}`);
  }

  // Path-identity: <provider>/<owner>/<name>.json must agree with the entry fields.
  // ns is "<provider>:<owner>", so the first two path segments rejoin with a colon.
  const [provider, owner, file] = relPath.split('/');
  const name = (file || '').replace(/\.json$/, '');
  if (entry.ns !== `${provider}:${owner}`) {
    throw new Error(`${where}: ns "${entry.ns}" doesn't match folder "${provider}/${owner}"`);
  }
